...
def validate_not_dup(people: list):
    card_set = set()
    for person in people:
        for card in person[1]:
            card_set.add(card)
    return len(card_set) == (len(people) * 5)
...
def validate_full_house(cards: list):
    ...
    b_same = numbers[2] == numbers[3] == numbers[4] and numbers[0] == numbers[1]
...
def main():
    ...
    for _ in range(int(input())):
        cards = get_cards()
        peoples.append([cards[0], cards[1:]])

    for person in peoples:
        if not validate_card(person[1]):
            print("Error input")
            return

    if not validate_not_dup(peoples):
        print("Duplicate deal")
        return

    peoples = [[person[0], get_score(person[1])] for person in peoples]

    peoples = sorted(peoples, key=lambda person: person[1], reverse=True)
    for person in peoples:
